...
export default function Page() {
  const [form, setForm] = useState({ name:'', email:'', lawFirm:'', message:'', agree:false, hp:'' })
  ...
  if (done) {
    return (
      <section className="container-p py-12 max-w-2xl">
        ...
        <p className="text-text-secondary">Skontaktujemy się wkrótce.</p>
        <OrgStamp />
      </section>
    )
  }
  ...
  return (
    <section className="container-p py-12 max-w-2xl">
      <h1 className="font-heading text-4xl mb-3">AI dla Prawników</h1>
      <p className="text-text-secondary mb-6">Automatyzacja pism, analiza umów, KPI — zostaw kontakt.</p>
      ...
      <form onSubmit={submit} className="card p-6 space-y-4">
        ...
        <div>
          <label className="block mb-1 text-sm text-text-secondary">Kancelaria</label>
          <input className="w-full p-3 rounded-xl bg-background-secondary/60 border border-white/10"
                 value={form.lawFirm} onChange={e=>setForm(s=>({ ...s, lawFirm:e.target.value }))}/>
        </div>
        <div>
