...
export default function Page(){
  const = useState({ name:'', email:'', message:'', agree:false, hp:'' })
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try{
      const res = await fetch('/api/lead-dla-ciebie', {
        ...
        body: JSON.stringify({ Name: state.name, Email: state.email, Message: state.message, hp: state.hp })
      });
...
| 'Błąd wysyłki') }
  ...
  return (
    <section className="container-p py-12 max-w-4xl">
      <h1 className="font-heading text-4xl mb-3">AI dla Twojej kariery: przewaga kompetencyjna</h1>
      <p className="text-text-secondary mb-6">Certyfikowane szkolenia AI z dofinansowaniem dla mieszkańców Warszawy. Krótko, praktycznie, z KPI.</p>
      <div className="grid md:grid-cols-2 gap-8">
        <div className="prose prose-invert"><h2>Co zyskujesz</h2><ul><li>Portfolio mini-projektów w 30 dni</li><li>Automatyzacja pracy i mierzalne KPI</li><li>Wsparcie w pozyskaniu finansowania</li></ul></div>
