...
export default function Page(){
  const = useState({ name:'', email:'', kancelaria:'', message:'', agree:false, hp:'' })
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try{
      const res = await fetch('/api/lead-prawnikow', {
        ...
        body: JSON.stringify({ Name: state.name, Email: state.email, Law_Firm: state.kancelaria, Message: state.message, hp: state.hp })
      });
...
| 'Błąd wysyłki') }
  ...
  return (
    <section className="container-p py-12 max-w-4xl">
      ...
      <p className="text-text-secondary mb-6">Program dla prawników i kancelarii. Automatyzacja analizy umów i pism, zachowując standardy etyczne.</p>
      <div className="grid md:grid-cols-2 gap-8">
        <div className="prose prose-invert"><h2>Korzyści</h2><ul><li>Analiza dokumentów i ekstrakcja klauzul</li><li>Szablony pism i checklisty jakości</li><li>Dofinansowanie (Warszawa), rozliczenia CPD</li></ul></div>
        <form onSubmit={submit} className="card p-6 space-y-4">
          ...
          <div><label className="block mb-1 text-sm text-text-secondary">Kancelaria (opcjonalnie)</label><input className="w-full p-3 rounded-xl bg-background-secondary/60 border border-white/10" value={state.kancelaria} onChange={e=>setState(s=>({...s, kancelaria:e.target.value}))}/></div>
