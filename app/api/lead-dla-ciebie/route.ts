...
export async function POST(req: Request){
  try {
    ...
    if (!url) throw new Error('Brak N8N_WEBHOOK_DLA_CIEBIE');
    const payload = {
      Name: data.Name, Email: data.Email, Message: data.Message?? null,
      Source: 'LP B2C', Intent_ID: 'LP_B2C', Verification_Status: 'pending',
