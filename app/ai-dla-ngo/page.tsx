...
export default function Page() {
  const [form, setForm] = useState({ name:'', email:'', org:'', message:'', agree:false, hp:'' })
  ...
  return (
    <section className="container-p py-12 max-w-2xl">
      <h1 className="font-heading text-4xl mb-3">AI dla NGO (Warszawa)</h1>
      <p className="text-text-secondary mb-6">Zostaw kontakt — wyślemy szczegóły programu i dofinansowania.</p>
      ...
      <form onSubmit={submit} className="card p-6 space-y-4">
        ...
        <div>
          ...
          <input className="w-full p-3 rounded-xl bg-background-secondary/60 border border-white/10"
                 value={form.org} onChange={e=>setForm(s=>({ ...s, org:e.target.value }))}/>
        </div>
        <div>
